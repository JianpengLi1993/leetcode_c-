...
public:
    ...
    2.从代码复杂度的考虑上讲，如果直接对两个数组进行merg(),再根据下标找，复杂度是O(m+n),为了更小，想变为log(m+n)级别，想到要从广义二分下手，一次O(1)操作，实现一半大量数据砍掉
        因此，想到用递归，要想找到第K小的元素，可以通过砍掉K/2个元素，使得要找的元素变为剩余数组中的第(K-K/2)小的元素，进而调用findKth((K-K/2));就像学生在班级排名为第10，当删掉前九排名的5人，
        他的排名就会变为第5.
    3.砍掉K/2个元素，为了不会砍掉真正要找的第K小的元素，在砍的过程是通过比较两个数组的第K/2个元素(下标为(start + k/2 -1))的大小来确定砍掉哪部分，即A[A_st + k / 2 - 1]与B[B_st + k / 2 - 1]比较;
    4.以上过程会遇到一些边界情况：
        4.1递归过程中的出口1，对于每个数组来说，当前轮的开始下标以及>=size(),即上一轮删完k/2个元素后，该数组刚好没了。
        这时，我们只需要返回另一个数组的第k(该层需要求的所谓的第k小的元素)个元素就是要找的最终中位数；
        4.2递归过程中的出口2，递归到k=1时，就可以直接比较两个数组当前层的首个元素较小的那一个就是最终要找的中位数。
